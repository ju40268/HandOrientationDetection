...
def save_img(img, index, filename):
   home = expanduser("~")
   #print 'now in save img'
   plt.imshow(img)
   plt.axis('off')
   #final_path = str(home) + '\output_contour_img '
   #if not os.path.exists(final_path):
   #    print ''
   #   os.makedirs(final_path)
   # plt.savefig(str(index)+".gif",bbox_inches='tight', pad_inches=0)
   #plt.imsave(final_path + filename + '_' + str(index) + '.jpg', img, cmap=plt.cm.GnBu)
   # plt.show()
...
def save_csv(data_numeric, num_frame, filename):
   ...
   with open(filename + '_' + 'difference_not_scale.csv', 'wb') as f:
       print f
